...
export class PoopService {
  ...
  getPoops(): Observable<Poop[]> {
    ...
    return this.http.get<Poop[]>(url, httpOptions).pipe(
      map((res) => {
  ...
  addRating(_id: string, likes: number, dislikes: number): void {
    ...
    this.http
      .post<any>(url, body, httpOptions)
      .subscribe((value) => console.log('returned this data ' + value));
  }
  ...
  addComment(_id: string, user: string, text: string, date: Date): void {
    ...
    console.log(
      'sending request with this data ' +
        body.text +
        '/' +
        body._id +
        '/' +
  ...
  checkOnline() {
    ...
    return this.http
      .get(url, httpOptions)
      .pipe(catchError(this.handleError<boolean>('isOnline', false)));
  }
