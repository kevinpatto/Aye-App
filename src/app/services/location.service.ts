  ...
  decodeLocation(longitude: number, latitude: number): any {
    ...
    return this.http.get<any>(url).pipe(
      map(data => {
        ...
        if (data.results[0].address_components[0] !== undefined && data.results[0].address_components[1] !== undefined) {
          street = data.results[0].address_components[0].long_name + " " + data.results[0].address_components[1].short_name;
        }
